...
Key 由本地 credential broker 解析，绝不放入 MCP 配置、参数或返回值。桌面端的已保存 Key 复制使用独立的启动期会话令牌，sidecar 直接写入系统剪贴板；该令牌不写入 Agent 网关发现文件，MCP 也不能调用复制接口。`probe_capability` 与生成工具可能调用外部模型并产生用量；声音复刻的 Schema 强制 `consent=true`。
